...
def test_net(name,
    ...
    for i in range(num_images):
        ...
        if visualize and np.random.rand() < 0.01:
            ...
            logger[0].add_image('task2_image', im2show, step)
    ...
    for i in range(20):
        logger[0].add_scalar('ap/class_{}'.format(i),
                          aps[0],
                          step)
    logger[1].plot('mAP', 'test', 'mAP value', step, np.mean(np.array(aps)))

    return aps


def main(logger, step):
    # load data
    ...
    trained_model = trained_model_fmt.format(cfg.TRAIN.SNAPSHOT_PREFIX, step)
    print('Loading {}'.format(trained_model))
    network.load_net(trained_model, net)
    print('load tmp successfully!')
    ...
    # evaluation
    aps = test_net(
        save_name, net, imdb, max_per_image, thresh=thresh, visualize=True, logger=logger, step=step)
