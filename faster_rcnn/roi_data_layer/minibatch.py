...
def get_weak_minibatch(roidb, num_classes):
    ...
    # this is like specigy that each image should contribute a specified number of roibd for training
    rois_per_image = cfg.TRAIN.BATCH_SIZE / num_images
    # defines half of the training roi should be foreground
    fg_rois_per_image = np.round(cfg.TRAIN.FG_FRACTION * rois_per_image)

    # Get the input image blob, formatted for caffe, im_blod is list of images and im_scales is a list of image scales
    # its not the actual size, but the amplified scale of the image
    ...
    for im_i in xrange(num_images):
        # Only two important information: overlaps and im_rois
        # im_rois: select the rois for training
        # Not sure about overlaps
        ...
        perm = np.random.permutation(np.arange(im_rois.shape[0]))
        perm_index = np.random.permutation(np.arange(rois_per_image))
        if rois_per_image>=im_rois.shape[0]:
            pass
        else:
            new_index = perm[perm_index]
            im_rois = im_rois[new_index, :]
        ...
        target = np.zeros((1, num_classes)).astype(np.float32)


        class_index = set(np.unique(roidb[im_i]["gt_classes"]) - 1)
        class_index.remove(-1)
        target[0, list(class_index)] = 1

        labels_blob = np.concatenate((labels_blob, target), 0)
